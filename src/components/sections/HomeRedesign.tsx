...
const process = [
  ...
  },
  {
    ...
    icon: Pencil,
    title: "Design",
    description: "I design the optimal system, architecture and user experience.",
  },
  {
    number: "03",
    icon: Code2,
...
const partnerItems = [
  {
    icon: Users,
    title: "Focused & independent",
    description: "You work directly with me from start to finish.",
  },
  {
    icon: Sparkles,
    title: "Technical excellence",
    description: "Clean code, solid architecture and future-proof solutions.",
  },
  {
    icon: BriefcaseBusiness,
    title: "Business-first thinking",
    description: "Solutions designed around your goals and outcomes.",
  },
  {
    icon: Workflow,
    title: "Long-term partner",
    description: "I stay involved to optimize, improve and scale with you.",
  },
] as const;
            ...
            {services.map((service) => {
              ...
              return (
                <article key={service.title} className="apple-glass group flex min-h-[270px] flex-col rounded-[24px] p-6 md:p-7">
                  ...
                  <h2 className="mt-7 max-w-[230px] text-[18px] font-semibold leading-[1.14] tracking-[-0.03em] text-[#101722]">
                    {service.title}
                  </h2>
                  <p className="mt-3 max-w-[300px] text-[12px] leading-5.5 text-[#677386]">
                    {service.description}
                  </p>
                  <div className="mt-auto pt-6">
                    <TextLink href="/work">Explore</TextLink>
                  </div>
                </article>
              );
            })}
          ...
          <div className="apple-glass rounded-[28px] p-4 sm:p-5 md:p-6">
            <div className="grid gap-4 xl:grid-cols-[0.72fr_1.35fr_0.93fr] xl:gap-5">
              <div className="flex min-h-[330px] flex-col rounded-[20px] border border-[#e1e6ec]/85 bg-white/64 p-5 md:p-6">
                ...
                <p className="mt-3 max-w-[260px] text-[11px] leading-5 text-[#677386]">
                  Complete rental management system for storage operations with automated billing, contracts and operations.
                </p>
              ...
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
                {supporting.map((study, index) => (
                  <Link
                    ...
                    className="group grid min-h-[156px] overflow-hidden rounded-[20px] border border-[#e1e6ec]/85 bg-white/66 p-4 transition hover:bg-white/82 sm:grid-cols-[1fr_0.8fr] xl:grid-cols-[1fr_0.85fr]"
                  >
                    <div className="flex flex-col pr-3">
                      <p className="text-[7px] font-semibold uppercase tracking-[0.15em] text-[#8993a2]">
                        {index === 0 ? "Automation" : "Web platform"}
                      </p>
                      <h3 className="mt-2 text-[13px] font-semibold leading-4 tracking-[-0.02em] text-[#101722]">
                        {study.company}
                      </h3>
                      ...
                      <span className="mt-auto inline-flex items-center gap-1.5 pt-3 text-[8px] font-semibold text-[#1d4ed8]">
                        View project <ArrowRight size={9} className="transition-transform group-hover:translate-x-0.5" />
                      </span>
                    </div>
          ...
          <div className="grid gap-4 xl:grid-cols-[1fr_1.55fr_0.72fr] xl:gap-5">
            ...
            <div>
              <div className="mb-4 text-center">
                <Eyebrow>Packages & Engagements</Eyebrow>
              </div>
              <div className="grid gap-3 md:grid-cols-3">
                {packages.map((pkg) => (
                  <article
                    key={pkg.title}
                    className={`apple-glass relative flex min-h-[310px] flex-col rounded-[24px] p-5 ${pkg.recommended ? "ring-1 ring-[#7ba8ee]/50" : ""}`}
                  >
                    {pkg.recommended ? (
                      <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 rounded-full border border-[#b9d4ff] bg-[#edf5ff] px-3 py-1 text-[6px] font-semibold uppercase tracking-[0.14em] text-[#2563eb] shadow-[0_5px_14px_rgba(37,99,235,0.10)]">
                        Most popular
                      </span>
                    ) : null}
                    ...
                    <ul className="mt-4 space-y-2">
                      {pkg.items.map((item) => (
                        <li key={item} className="flex items-start gap-2 text-[8px] leading-4 text-[#687486]">
                          <Check size={10} className="mt-0.5 shrink-0 text-[#3e70c9]" />
                          {item}
                        </li>
                      ))}
                    </ul>
            ...
            <article className="apple-glass flex min-h-[360px] flex-col items-center justify-center rounded-[26px] p-6 text-center">
              ...
              <h2 className="mt-5 text-balance text-[20px] font-semibold leading-[1.08] tracking-[-0.035em] text-[#101722]">
                Ready to bring order to operations and build systems that scale with your business?
              </h2>
              <Link
                href="/contact"
                className="group mt-7 inline-flex h-11 items-center gap-2 rounded-full bg-[#07111d] px-5 text-[11px] font-semibold text-white shadow-[0_12px_30px_rgba(7,17,29,0.18)] transition hover:-translate-y-0.5 hover:bg-[#0c1d30]"
              >
